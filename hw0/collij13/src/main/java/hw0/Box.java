...
public class Box implements Iterable<Ball> {
    ...
    public boolean add(Ball b) {
        // Your code goes here.  Remove the exception after you're done.
        double volume = ballContainer.getVolume();
        if((volume + b.getVolume()) <= this.max_vol) {
    ...
    public Iterator<Ball> getBallsFromSmallest() {
    	...
    	Iterator<Ball> x = ballContainer.iterator();
    	
    	while(x.hasNext()) {
    		list.add(x.next());
    	}
    	// https://dzone.com/articles/java-8-comparator-how-to-sort-a-list
    	// this is where I found how to sort this way
    	list.sort(Comparator.comparing(Ball::getVolume));
    	
    	Iterator<Ball> out = list.iterator();
